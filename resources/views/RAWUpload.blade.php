...
<head>
    ...
    <title>Upload Raw Excel File</title>
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen p-6 max-w-7xl w-full">
    <div class="w-full max-w-5xl bg-white shadow-lg rounded-xl p-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800 mb-4">Upload Raw Excel File</h2>
        ...
        {{-- Upload Form --}}
        <form id="uploadForm" action="{{ route('raw.upload.process') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <label class="block">
                <span class="text-gray-700">Choose a Raw Excel file</span>
        ...
        @if(isset($data) && $data->count() > 0)
            <h3 class="text-xl font-semibold text-gray-800 mt-6">Stored Raw Data</h3>

            <div class="flex justify-center gap-8 mt-6">
                <a href="{{ route('download.excel', ['type' => 'raw']) }}" 
